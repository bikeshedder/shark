...
class ItemForm(forms.ModelForm):
    customer_number = forms.CharField(
        max_length=Customer._meta.get_field("number").max_length
    )
    ...
    def resolve_customer(self, customer_dict):
        ...
        if not customer:
            ...
            self.errors["customer_number"].append(gettext("Customer does not exist."))
...
class ImportItemsForm(forms.Form):
    ...
    encoding = forms.ChoiceField(
        choices=(("utf-8", "utf-8 (LibreOffice)"), ("latin1", "latin1 (Excel)"))
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        file_field.help_text = format_lazy(
            "{} {}", file_field.help_text, ", ".join(ItemForm._meta.fields)
        )

    def clean(self):
        super(ImportItemsForm, self).clean()
        if (
            "file" not in self.cleaned_data
            or "delimiter" not in self.cleaned_data
            or "encoding" not in self.cleaned_data
        ):
            return
        item_forms = []
        errnous_rows = []
        ...
        if errnous_rows:
            ...
            for row, errnous_fields in errnous_rows:
                ...
                for field_name, field_errors in errnous_fields.items():
                    row_errors.append(
                        "<li>%s: %s</li>" % (field_name, " ".join(field_errors))
                    )
                errors.append(
                    mark_safe(
                        "%s %d:<ul>%s</ul>"
            ...
            self.cleaned_data["items"] = [
                item_form.save(commit=False) for row, item_form in item_forms
            ]
